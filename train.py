...
def train_one_window(config, workdir, model, res_sampler, u_ref, v_ref, w_ref, p_ref, idx):
    ...
    for step in range(config.training.max_steps):
        ...
        if jax.process_index() == 0:
            if step % config.logging.log_every_steps == 0:
                ...
                log_dict = evaluator(state, batch, u_ref, v_ref, w_ref, p_ref)
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    for idx in range(config.training.num_time_windows):
        ...
        model = train_one_window(
            config, workdir, model, res_sampler, u_star, v_star, w_star, p_star, idx
        )
